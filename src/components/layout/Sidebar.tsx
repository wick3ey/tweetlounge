...
import { Home, Search, Bell, Mail, User, Hash, Settings, BarChart2, Compass, Bookmark, Shield } from 'lucide-react'
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="h-screen bg-crypto-darkgray border-r border-crypto-gray/40 w-16 flex flex-col items-center pt-4 crypto-gradient-bg shadow-lg">
      <div className="mb-8">
          ...
          {menuItems.map((item) => (
            <Tooltip key={item.path}>
              <TooltipTrigger asChild>
                <Link 
                  to={item.path} 
                  className={`crypto-sidebar-item ${currentPath === item.path ? 'active text-crypto-blue' : 'text-crypto-lightgray'}`}
                >
                  <item.icon className="w-5 h-5" />
                </Link>
              </TooltipTrigger>
              <TooltipContent side="right" className="bg-crypto-darkgray border-crypto-gray/60 text-white">
                {item.label}
              </TooltipContent>
            </Tooltip>
          ))}
