        ...
        <TabsContent value="media" className="mt-0 pt-2">
          <ScrollArea className="h-[calc(100vh-250px)] pr-4">
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 pb-6">
                {mediaTweets.map((tweet) => (
                  <div key={tweet.id} className="bg-crypto-darkgray border border-crypto-gray rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 hover:border-crypto-blue/40">
                    {tweet.image_url && (
                      <img 
                        src={tweet.image_url} 
                        alt="Tweet media" 
